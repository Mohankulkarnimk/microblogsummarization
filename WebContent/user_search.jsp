...
<div class="ch-container">
	...
	<div class="row">        
		...
		%>
            ...
            <h2 align="center">Search Followers Here!...</h2>
        </div>
        <!--/span-->
        ...
        <div class="well col-md-4 center login-box">
            ...
            <form action="UserSearchController" method="post">
                <fieldset>
					...
					<br>
					<%
						String errMsg1 = (String)session.getAttribute("ErrMsgRecord");
						if(errMsg1!=null && errMsg1!=""){
					%>
						<p style="color:black"><%=errMsg1 %></p>
					<%
						}
					%>
					<hr>
                   ...
                   <%
                   		ArrayList<User> listUser = (ArrayList<User>)session.getAttribute("friendlist");
                   if(listUser!=null){
                   	...
                   	<br>
                   	<hr>
                   				<div><%=listUser.get(i).getFname()+" "+listUser.get(i).getLname() %></div>
                   				<div><%=listUser.get(i).getAddress() %></div>
                   				<a href="SendRequestController?followerId=<%=listUser.get(i).getUserId()%>&userId=<%=userId %>" style="color:blue">Follow</a>
                   	 <%
						String sucMsg = (String)session.getAttribute("SucMsgRequest");
                   	 	Object fid = session.getAttribute("followerId");
                   	 	if(fid!= null){
                   	 	int followerId = Integer.parseInt(fid.toString());
							if(sucMsg!=null && sucMsg!="" && followerId==listUser.get(i).getUserId()){
					%>
